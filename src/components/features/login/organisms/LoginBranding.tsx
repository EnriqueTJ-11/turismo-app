...
const LoginBranding: React.FC = () => {
  return (
    <div className="relative hidden w-full lg:flex lg:w-1/2 xl:w-3/5 overflow-hidden group">
      {/* Background Image */}
      <div className="absolute inset-0">
        <Image
          ...
          alt="Paisaje amazónico de fondo"
          title="Paisaje amazónico de fondo"
          fill
          sizes="50vw"
          ...
          <div className="flex items-center gap-6 pt-4">
            <div className="flex -space-x-3">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-8 w-8 rounded-full border-2 border-white overflow-hidden shadow-lg"
                >
                  <Image
                    alt={`Explorador ${i}`}
                    title={`Explorador ${i}`}
            ...
            <p className="text-xs font-bold tracking-wide uppercase">
              Únete a más de 5,000 aventureros
            </p>
          </div>
        </div>
